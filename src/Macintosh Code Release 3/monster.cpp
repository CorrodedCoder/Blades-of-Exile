...
//			format_to_buf(debug,"  t: {:d} targets {:d}.",i,monst_target[i]);
			...
			if ((c_town.monst.dudes[i].active == 2)
				...
				if (((c_town.monst.dudes[i].attitude == 0) || (monst_target[i] == 6)) && (c_town.hostile == 0)) {
					if (c_town.monst.dudes[i].mobile == TRUE) { // OK, it doesn't see the party or
					    ...
					    if ((c_town.monst.dudes[i].attitude % 2 != 1) || (rand_short(0,1) == 0)) {
...
short monst_pick_target(short which_m)
{
	...
	if (monst_target[which_m] < 6) 
		if ((adven[monst_target[which_m]].main_status != status_type::Normal) || (rand_short(0,3) == 1))
			monst_target[which_m] = 6;		
	
	if (is_combat() && (cur_monst->attitude % 2 == 1)) {
		if (spell_caster < 6)
			if ((rand_short(1,5) < 5) && (monst_can_see(which_m,pc_pos[spell_caster]) == TRUE)
					&& (adven[spell_caster].main_status == status_type::Normal)) 
						return spell_caster;
		if (missile_firer < 6)
			if ((rand_short(1,5) < 3) && (monst_can_see(which_m,pc_pos[missile_firer]) == TRUE)
				&& (adven[missile_firer].main_status == status_type::Normal)) 
					return missile_firer;
		if (monst_target[which_m] < 6)
			if ((monst_can_see(which_m,pc_pos[monst_target[which_m]]) == TRUE)
				&& (adven[monst_target[which_m]].main_status == status_type::Normal)) 
...
//		}
	...
	if ((dist(cur_monst->m_loc,c_town.monst.dudes[targ_m - 100].m_loc) ==
		dist(cur_monst->m_loc,pc_pos[targ_pc])) && (rand_short(0,6) < 3))
			return targ_m;
			else return targ_pc;
	if (dist(cur_monst->m_loc,c_town.monst.dudes[targ_m - 100].m_loc) <
...
short monst_pick_target_monst(creature_data_type *which_m)
{
	...
	for (i = 0; i < T_M; i++) {
		if ((c_town.monst.dudes[i].active > 0) && // alive
			 ...
			 ((dist(which_m->m_loc,c_town.monst.dudes[i].m_loc) == min_dist) && (rand_short(0,7) < 4))) &&
...
short monst_pick_target_pc(short m_num,creature_data_type *which_m)////
{
	...
	r1 = rand_short(0,5);
	while ((num_tries < 6) && ((adven[r1].main_status != status_type::Normal) ||
...
short select_active_pc()
{
	...
	r1 = rand_short(0,5);
	while ((adven[r1].main_status != status_type::Normal) && (num_tries++ < 50))
		r1 = rand_short(0,5);
...
short closest_pc(location where)
{
	...
	for (i = 0; i < 6; i++)
		if ((adven[i].main_status == status_type::Normal) && (dist(where,pc_pos[i]) < how_close)) {
			store = i;
...
short switch_target_to_adjacent(short which_m,short orig_target)
{
	...
	if (is_combat() && (orig_target < 6))
		if ((adven[orig_target].main_status == status_type::Normal) && (monst_adjacent(pc_pos[orig_target],which_m) == TRUE))
			return orig_target;
	if (orig_target >= 100)
		if ((c_town.monst.dudes[orig_target - 100].active > 0) &&
		 (monst_adjacent(c_town.monst.dudes[orig_target - 100].m_loc,which_m) == TRUE))
	...
	if (is_combat())
		for (i = 0; i < 6; i++)
			if ((adven[i].main_status == status_type::Normal) && (monst_adjacent(pc_pos[i],which_m) == TRUE) && 
	...
	i = 0;
	num_adj = rand_short(1,num_adj);
	while ((num_adj > 1) || (adven[i].main_status != status_type::Normal) || (monst_adjacent(pc_pos[i],which_m) == FALSE)) {
		if ((adven[i].main_status == status_type::Normal) && (monst_adjacent(pc_pos[i],which_m) == TRUE)) 
			num_adj--;
		i++;
		}
...
Boolean rand_move(char i)
{
	...
	if (acted_yet == FALSE) {
		...
		for (j = 0; j < 3; j++) {
			...
			store_loc.x += rand_short(0,24) - 12;
			store_loc.y += rand_short(0,24) - 12;
		...
		if (monster_targs[i].x == 0) {
			...
			if ((loc_off_act_area(store_loc) == FALSE) && (rand_short(0,1) == 1))
				monster_targs[i] = store_loc;
				else {
				store_loc = c_town.monst.dudes[i].m_loc;
				store_loc.x += rand_short(0,20) - 10;
				store_loc.y += rand_short(0,20) - 10;
...
location random_shift(location start)
{
	...
	store = start;
	store.x = store.x + rand_short(0,2) - 1;
	store.y = store.y + rand_short(0,2) - 1;
...
//mode; // 1 - town 2 - combat
{
	...
	////
	which_m = &c_town.monst.dudes[which_monst];
	ter_abil = scenario.ter_type(ter).special;
	...
	if (which_m->m_d.spec_skill == 13)
		guts = 20;
		else guts = rand_short(1,(which_m->m_d.level / 2));
	...
	if (is_fire_barrier(where_check.x,where_check.y)) {
		if ((which_m->attitude % 2 == 1) && (rand_short(0,100) < (which_m->m_d.mu * 10 + which_m->m_d.cl * 4))) {
			play_sound(60);
...
void record_monst(creature_data_type *which_m)
{
	...
	if ((which_m->m_d.x_width > 1) || (which_m->m_d.y_width > 1)) {
		...
		play_sound(68);
			...
			format_to_buf(str,"  Caught in slot {:d}.",r1 + 1);
